...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    if (req.method === 'GET') {
      const { ok, status, body } = await fetchPublicReviews('review02');
      if (!ok) {
    ...
    if (req.method === 'POST') {
      ...
      const { ok, status, body } = await forwardMasterclassReview(data, 'review02');
      if (!ok) {
    ...
    console.error('[reviews02] error:', message);
