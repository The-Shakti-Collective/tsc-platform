...
export default function KineticTeamHybrid({
  ...
  const bgClass = darkMode ? 'bg-neutral-950 text-neutral-200' : 'bg-cream text-charcoal';
  ...
  return (
    <div
      ...
      className={`relative min-h-screen w-full cursor-default px-6 py-24 ${bgClass} md:px-12`}
    >
      {/* Background Ambience */}
        ...
        >
          <div>
            <h1 className={`text-4xl font-light tracking-tighter ${
              ...
              {title} <span className={darkMode ? 'text-neutral-600' : 'text-slate-400'}>{subtitle}</span>
            </h1>
          </div>
          <div className={`h-px flex-1 ${darkMode ? 'bg-neutral-900' : 'bg-cream-dark'} mx-8 hidden md:block`} />
          ...
          <AnimatePresence mode="wait">
            {activeId && (
              <motion.div
                ...
                className={`relative h-64 w-80 overflow-hidden rounded-xl border ${
                  darkMode ? 'border-white/10 bg-neutral-900' : 'border-charcoal/20 bg-cream-light'
                } shadow-2xl`}
              >
...
function TeamRow({
  ...
  return (
    <motion.div
      ...
      className={`group relative border-t ${borderClass} transition-colors duration-500 last:border-b ${
    ...
    >
      <div className="relative z-10 flex flex-col py-8 md:flex-row md:items-center md:justify-between md:py-12">
        ...
        <div className="mt-4 flex items-center justify-between pl-12 pr-4 md:mt-0 md:justify-end md:gap-12 md:pl-0 md:pr-0">
          ...
          {/* Mobile Toggle Icon */}
          <div className={`block md:hidden ${darkMode ? 'text-neutral-500' : 'text-slate-400'}`}>
          ...
          <motion.div
            ...
            className={`hidden md:block ${darkMode ? 'text-white' : 'text-charcoal'}`}
          >
      ...
      <AnimatePresence>
        {isMobile && isActive && (
          <motion.div
            ...
            className={`overflow-hidden ${darkMode ? 'bg-neutral-900/50' : 'bg-cream-dark/30'}`}
          >
            <div className="p-4">
              <div className="relative aspect-video w-full overflow-hidden rounded-lg">
                ...
                <div className="absolute bottom-4 left-4">
                  <p className="text-xs uppercase tracking-widest text-white">{data.bio || data.role}</p>
                </div>
              </div>
            </div>
          </motion.div>
