...
function toDateKey(value, timeZone = DEFAULT_TZ) {
...
function getTodayDateKey(timeZone = DEFAULT_TZ) {
  return toDateKey(new Date(), timeZone);
}

function assertDateKeyNotBeforeToday(dateKey, { label = 'Date', timeZone = DEFAULT_TZ } = {}) {
...
function validateTaskTimelineFields(fields, timeZone = DEFAULT_TZ) {
  const checks = [
...
function buildDateTimeFromParts(dateKey, timeStr = '00:00', tzOffset = '+05:30') {
...
function validateCalendarEventRange(
