/** @typedef {{ _id: string, name?: string, email?: string }} MentionUser */
...
const tokenizeMentionText = (text) => {
  if (!text) return [];
...
const extractUserMentionLabels = (text) => {
...
const resolveUserByLabel = (label, users = []) => {
...
const resolveAssetByLabel = (label, assets = []) => {
  const q = normalizeLabel(label);
  if (!q) return null;
...
const buildDisplaySegments = (text, users = [], assets = []) =>
...
const extractUserMentionLabelsFromFields = (...texts) => {
...
module.exports = {
  ...
  extractUserMentionLabelsFromFields,
  resolveUserByLabel,
  resolveAssetByLabel,
  tokenizeMentionText,
