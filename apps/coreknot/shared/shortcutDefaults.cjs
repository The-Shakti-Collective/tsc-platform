/**
 * Bindable keyboard actions — defaults shared by client + server validation.
 * CommonJS for Node server only. Client uses client/src/lib/shortcutDefaultsCore.js — keep both in sync.
 * keys: mod = Ctrl on Windows/Linux, Cmd on macOS; single-char = lowercase letter.
 */
...
const NAV_ACTIONS = NAV_ROUTES.map((route) => ({
  id: `nav-${route.suffix}`,
...
const SHORTCUT_ACTIONS = [...GLOBAL_ACTIONS, ...QUICK_ACTIONS, ...NAV_ACTIONS];

const SYSTEM_SHORTCUT_CONFLICTS = [
...
function normalizeKeyTokens(keys) {
...
function getDefaultBindingsMap() {
  const map = {};
...
function mergeShortcutBindings(overrides = {}) {
...
function filterActionsForUser(actions, { isAdmin = false } = {}) {
  return actions.filter((a) => !a.adminOnly || isAdmin);
}

function buildLegacyGChordRoutes(bindingsMap, { isAdmin = false } = {}) {
  const routes = {};
